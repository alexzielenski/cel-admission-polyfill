...
type PolicyTemplateLister interface {
	...
	List(selector labels.Selector) (ret []*v0alpha2.PolicyTemplate, err error)
...
// List lists all PolicyTemplates in the indexer.
func (s *policyTemplateLister) List(selector labels.Selector) (ret []*v0alpha2.PolicyTemplate, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v0alpha2.PolicyTemplate))
	})
	return ret, err
}
...
type PolicyTemplateNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v0alpha2.PolicyTemplate, error)
...
// List lists all PolicyTemplates in the indexer for a given namespace.
func (s policyTemplateNamespaceLister) List(selector labels.Selector) (ret []*v0alpha2.PolicyTemplate, err error) {
...
// Get retrieves the PolicyTemplate from the indexer for a given namespace and name.
func (s policyTemplateNamespaceLister) Get(name string) (*v0alpha2.PolicyTemplate, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v0alpha2.Resource("policytemplate"), name)
	}
	return obj.(*v0alpha2.PolicyTemplate), nil
}
