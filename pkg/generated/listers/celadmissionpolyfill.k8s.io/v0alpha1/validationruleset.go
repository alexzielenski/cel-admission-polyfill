...
type ValidationRuleSetLister interface {
	...
	List(selector labels.Selector) (ret []*v0alpha1.ValidationRuleSet, err error)
...
// List lists all ValidationRuleSets in the indexer.
func (s *validationRuleSetLister) List(selector labels.Selector) (ret []*v0alpha1.ValidationRuleSet, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v0alpha1.ValidationRuleSet))
	})
	return ret, err
}
...
type ValidationRuleSetNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v0alpha1.ValidationRuleSet, error)
...
// List lists all ValidationRuleSets in the indexer for a given namespace.
func (s validationRuleSetNamespaceLister) List(selector labels.Selector) (ret []*v0alpha1.ValidationRuleSet, err error) {
...
// Get retrieves the ValidationRuleSet from the indexer for a given namespace and name.
func (s validationRuleSetNamespaceLister) Get(name string) (*v0alpha1.ValidationRuleSet, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v0alpha1.Resource("validationruleset"), name)
	}
	return obj.(*v0alpha1.ValidationRuleSet), nil
}
