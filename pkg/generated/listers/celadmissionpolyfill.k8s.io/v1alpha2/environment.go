...
type EnvironmentLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha2.Environment, err error)
...
// List lists all Environments in the indexer.
func (s *environmentLister) List(selector labels.Selector) (ret []*v1alpha2.Environment, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha2.Environment))
	})
	return ret, err
}
...
type EnvironmentNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha2.Environment, error)
...
// List lists all Environments in the indexer for a given namespace.
func (s environmentNamespaceLister) List(selector labels.Selector) (ret []*v1alpha2.Environment, err error) {
...
// Get retrieves the Environment from the indexer for a given namespace and name.
func (s environmentNamespaceLister) Get(name string) (*v1alpha2.Environment, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha2.Resource("environment"), name)
	}
	return obj.(*v1alpha2.Environment), nil
}
